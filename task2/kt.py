def run_test(n, start_x, start_y, test_num, short_f, long_f):
    ...
    # Initial call to try_move with L=2, depth=1
...
def main():
    ...
    open("out-short.txt", "w").close()
    open("out-long.txt", "w").close()
    for i, (n, x, y) in enumerate(tests, 1):
        with open("out-short.txt", "a") as short_f, open("out-long.txt", "a") as long_f:
            if i > 1:
                # Add a separator between test cases
                short_f.write("\n")
                long_f.write("\n")
            run_test(n, x, y, i, short_f, long_f)
